            ...
            LazyColumn(
                ...
                version("0.6", LocalDate(2026, 9, 27)) {
                    append(
                        ...
                        - 设置页新增「TTS 缓存」独立页面：列出每条语音，可试听 / 删除 / 一键清空
                        - 支持提前预热：多行粘贴词语批量缓存，或直接选一个牌组
                        - 缓存支持导出 / 导入 zip，换设备或重装前可以先备份
                        - 朗读按钮改成只在合成期间显示加载圈，反复点可以打断重读
                        - 修复 TTS 缓存页弹窗底色异常（改用应用统一的对话框样式）
                        - 读牌组不再逐个查词典，几百个词的牌组不用再等几分钟
                        """.trimIndent()
                    )
                }
