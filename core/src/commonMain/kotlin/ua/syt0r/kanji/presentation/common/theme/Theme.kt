...
private val AmoledDim = Color(0xFF121212)

// 从 seed 生成的 ColorScheme 构造最终的 material3 scheme：
// 背景/表面/容器全部随 seed 派生，on* 色由算法保证对比（深背景白字/浅背景黑字），避免白字白背景
private fun buildColorScheme(seeded: ColorScheme, dark: Boolean, amoled: Boolean): ColorScheme {
    val background = if (amoled) AmoledBackground else seeded.background
    val surface = if (amoled) AmoledBackground else seeded.surface
    val container = if (amoled) AmoledBackground else seeded.surfaceVariant
    val surfaceDim = if (amoled) AmoledDim else seeded.surface
    val surfaceBright = if (amoled) AmoledDim else seeded.surface
    ...
    return if (dark) {
        darkColorScheme(
            ...
            background = background, onBackground = seeded.onBackground,
            surface = surface, onSurface = seeded.onSurface,
            surfaceVariant = container, onSurfaceVariant = seeded.onSurfaceVariant,
            ...
            surfaceContainer = container, surfaceContainerHigh = container,
            surfaceContainerHighest = container, surfaceContainerLow = surface,
...
fun AppTheme(
    ...
    val colorScheme = buildColorScheme(seeded, isDark, useAmoledTheme)
